...
plot_stackedHistogram({'Size','[Re]','Closest Distance'},...
    get_data('all','Size'),2,'Size',...
    get_data('all','ClosestDistance'),3,'ClosestDistance')
...
plot_histogram({'Distance to CS-BS Connection Point','[Re]'},0:1.5:9,...
...
plot_histogram({'Event Width','[Re]'},0:3:15,...
...
plot_histogram({'CS-BS Angle','[\Theta]'},10,...
...
plot_histogram({'Shock Angle Downstream','[\theta]'},0:15:90,...
...
plot_histogram({'Shock Angle Upstream','[\theta]'},0:15:90,...
...
plot_scatter({'Shock Angle Downstream','Shock Angle Upstream','Event Area'},...
    angle90(get_data(type1,'Shock Angle Before')),angle90(get_data(type1,'Shock Angle After')),name1,...
    angle90(get_data(type2,'Shock Angle Before')),angle90(get_data(type2,'Shock Angle After')),name2,...
    35*get_data(type1,'Event Area'),35*get_data(type2,'Event Area'),'Event Area')
...
plot_scatter({'E-CS Angle Downstream','E-CS Magnitude Downstream','Event Area'},...
    ...
    10*get_data(type1,'Event Area'),10*get_data(type2,'Event Area'),'Event Area')
...
plot_scatter({'E-CS Angle Upstream','E-CS Magnitude Upstream','Event Area'},...
    ...
    20*get_data(type1,'Event Area'),20*get_data(type2,'Event Width'),'Event Area')
...
%Mach Number
plot_histogram({'Mach Number Downstream',''},8:4:24,...
    get_data(type1,'Mach Number Before'),name1,...
    get_data(type2,'Mach Number Before'),name2)

plot_histogram({'Mach Number Upstream',''},8:4:24,...
...
plot_histogram({'Normalized Mach Number Downstream',''},8:4:20,...
    get_data(type1,'Mach Number Before'),name1,...
    get_data(type2,'Mach Number Before'),name2, MachNumber)

plot_histogram({'Normalized Mach Number Upstream',''},8:4:20,...
    get_data(type1,'Mach Number After'),name1,...
...
plot_histogram({'Solar Wind Magnetic Field Magnitude','[nT]'},0:2:12,...
    Bdatamag,'WIND')
...
plot_histogram({'Magnetic Field Downstream','[nT]'},0:3:12,...
    get_data(type1,'Bpre'),name1,get_data(type2,'Bpre'),name2)

plot_histogram({'Magnetic Field Upstream','[nT]'},0:3:12,...
    get_data(type1,'Bpost'),name1,get_data(type2,'Bpost'),name2)

plot_histogram({'Normalized Magnetic Field Downstream','[nT]'},0:3:12,...
    get_data(type1,'Bpre'),name1,get_data(type2,'Bpre'),name2,Bdatamag)

plot_histogram({'Normalized Magnetic Field Upstream','[nT]'},0:3:12,...
...
function [data] = get_data(Event_Type,Event_Parameter)
    ...
    Database_Directory = '~/Library/Mobile Documents/com~apple~CloudDocs/Research/Analysis/EventParameterDatabase_Typical.xlsx';
