...
% %     a10 =  b10(1) +  b10(2)*M_A + b10(3)/(M_A-1)^2;
    ...
    [leading_shock_normal] = calculate_shockNormal(a1,a3,a4,a7,a8,leading_bs_pos);
    [trailing_shock_normal] = calculate_shockNormal(a1,a3,a4,a7,a8,trailing_bs_pos);
...
function [intersection_point,Leading_DistanceFromIntersectiontoBS,Trailing_DistanceFromIntersectiontoBS] = calculate_EdgesPlaneIntersection (spacecraft_trajectory_plane,leading_plane,leading_bs_pos,trailing_plane,trailing_bs_pos)
    syms x y z 
    X = solve(leading_plane==trailing_plane,x);
    A = subs(leading_plane,x,X);
    Y = solve(A,y);
    
    B = subs(spacecraft_trajectory_plane,x,X);
    C = subs(B,y,Y);
    ZZ = solve(C,z);
...
%% Calculate Bow Shock Normal at Connection Point
function [shock_normal] = calculate_shockNormal(a1,a3,a4,a7,a8,bs_pos)
...
function [ClosestDistance,closest_point] = calculate_closestDistance(merka_bowshock, core_position,a1,a3,a4,a7,a8)

    %Solve for the minimum distance from the HFA to the BS with lagrange multipliers
    syms x y z lambda
    dLdx = 2*x - 2*core_position(1) + 2*lambda*a1*x + 2*lambda*a4*y + 2*lambda*a7;
    dLdy = 2*y - 2*core_position(2) + 2*lambda*y + 2*lambda*a4*x + 2*lambda*a8;
    dLdz = 2*z - 2*core_position(3) + 2*lambda*a3*z;
