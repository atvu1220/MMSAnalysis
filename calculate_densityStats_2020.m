function [substructurePresent,fpi_coreTimedata,fpi_coreNdata,trendDensity,Sigma,substructureStarts,substructureEnds,...
        ...
        SS_i_nTCorr,SS_e_nTCorr,SSSize_in_IonScales,maxNsigma,ionGyroradius,ionInertiallength,...
        SSSizeBulkVflow_electronGyro,electronGyroradius,electronInertiallength] = ...
    ...
    %Code now uses fpi_eNdata instead of fpi_iNdata for SS identification
    %fpi_e_timedata,fpi_e_ndata
    
    
    coreStarte_index = find(fpi_e_timedata > coreStart, 1);
    coreEnde_index = find(fpi_e_timedata > coreEnd, 1);
    
    fpi_e_coreTimedata = fpi_e_timedata(coreStarte_index:coreEnde_index,1);
    fpi_e_coreNdata = fpi_e_ndata(coreStarte_index:coreEnde_index,1);
    fpi_e_coretparadata = fpi_e_tparadata(coreStarte_index:coreEnde_index,1);
    fpi_e_coretperpdata = fpi_e_tperpdata(coreStarte_index:coreEnde_index,1);
    
    p = polyfit(fpi_e_coreTimedata,fpi_e_coreNdata,1); %parameters for linear fit
    trendDensity = polyval(p,fpi_e_coreTimedata); %Trend Line
    Sigma = std(fpi_e_coreNdata - trendDensity); %1 STD
    
    %Find max Density, then find sigma
    maxN = max(fpi_e_coreNdata-trendDensity);
    maxNsigma = (maxN)/Sigma;
    ...
    transitions = diff([0;((fpi_e_coreNdata >= ssSTDs*Sigma+trendDensity))~=0;0]);
    ...
    if ~isempty(substructureStarts)
        ...
        if length(substructureStarts) > 1
            ...
            for i=1:length(substructureStarts)
                if ~(substructureStarts(i) >= length(fpi_e_coreTimedata)) && ~(substructureEnds(i) > length(fpi_e_coreTimedata))
                    
                    maxNofEachSS = [maxNofEachSS,max(...
                        fpi_e_coreNdata(min([substructureStarts(i),substructureEnds(i)])...
        ...
        %         end
          ...
          %% Find the entire time period of substructure
            
            if (substructureStarts_index >= length(fpi_e_coreTimedata) || substructureEnds_index > length(fpi_e_coreTimedata) || substructureStarts_index == 1 || substructureEnds_index == length(fpi_e_coreTimedata)) %Case no SS
                
            else %Case of SS, find the entire time period
            %Find Starting point of SS
            index = substructureStarts_index;
            while ( (fpi_e_coreNdata(index) > trendDensity(index)) || (index == 1) )
                index = index - 1;
            end
            substructureStarts_index = index;
            
            
            %Find Starting point of SS
            index = substructureEnds_index;
            while ( (fpi_e_coreNdata(index) > trendDensity(index)) || (index == length(fpi_e_coreNdata)) )
                index = index + 1;
                
                if index > length(fpi_e_coreNdata)
                    %The substructure is attached to the edge. this is considered not a substructure
                    %because it doesn't return to the average before the core is overujvjky
                    break;
                end
                
            end
            substructureEnds_index = index;
        ...
        %a SS
        if (substructureStarts_index >= length(fpi_e_coreTimedata)) || (substructureEnds_index > length(fpi_e_coreTimedata)) || substructureStarts_index == 1 || substructureEnds_index == length(fpi_e_coreTimedata)
            ...
            ionGyroradius = {[]};
            ionInertiallength = {[]};
            SSSizeBulkVflow_electronGyro = {[]};
            electronGyroradius = {[]};
            electronInertiallength = {[]};
            
            %Plot densities anyways.
            pi = polyfit(fpi_coreTimedata,fpi_coreNdata,1); %parameters for linear fit
            trendDensityi = polyval(pi,fpi_coreTimedata); %Trend Line
            Sigmai = std(fpi_coreNdata - trendDensityi); %1 STD
            screenSize = get(0,'ScreenSize');
            figure('Position',[0 screenSize(4) 550 550])
            %     co = [0 0 1;
            %         0 1 0;
            %         1 0 0;
            %         0 0 0];
            ...
            set(gcf,'color','w');
            plot(fpi_coreTimedata,fpi_coreNdata,'r'); hold on;
            %plot(fpi_coreTimedata(substructureStarts_index:substructureEnds_index),fpi_coreNdata(substructureStarts_index:substructureEnds_index),'r','linewidth',2);
            plot(fpi_e_coreTimedata,fpi_e_coreNdata,'b');
            %plot(fpi_e_coreTimedata(substructureStartse_index:substructureEndse_index),fpi_e_coreNdata(substructureStartse_index:substructureEndse_index),'b','linewidth',2);
            
            plot(fpi_e_coreTimedata,trendDensity,'k','linewidth',2);
            plot(fpi_e_coreTimedata,ssSTDs*Sigma+trendDensity,'k','linewidth',2);
            
            plot(fpi_coreTimedata,trendDensityi,'g','linewidth',2);
            plot(fpi_coreTimedata,ssSTDs*Sigmai+trendDensityi,'g','linewidth',2);
            
            datetick
            set(gca,'XMinorTick','off','TickDir','out','YMinorTick','on','linewidth',2)
            fileName = 'coreDensities_noSS_';
            title('Core Densities_noSS_')
            % cd '~/Library/Mobile Documents/com~apple~CloudDocs/Research/Analyzed Events/All Events_Typical_Final'
            ...
            %             maxN_FPItime = fpi_e_coreTimedata(fpi_e_coreNdata == max(fpi_e_coreNdata(substructureStarts_index:substructureEnds_index)));
            SSStart_e_FPItime = fpi_e_coreTimedata(substructureStarts_index,1);
            SSEnd_e_FPItime = fpi_e_coreTimedata(substructureEnds_index,1);
            %Change e times to i times.
            substructureStartse_index = substructureStarts_index;
            substructureEndse_index = substructureEnds_index;
            substructureStarts_index = find(fpi_coreTimedata<=SSStart_e_FPItime,1,'last');
            substructureEnds_index = find(fpi_coreTimedata>=SSEnd_e_FPItime,1);
            
            
            %Downsample Electron Density to Ion Density Time Scale, use electron density for ion
            %density
% % % % % % %             [~,fpi_coreNdata] = interpxyz(fpi_e_coreTimedata,fpi_e_coreNdata,fpi_coreTimedata);
            ...
            substructureStarts_index_e = substructureStartse_index;%find(fpi_coreTimedata(substructureStarts_index) >= fpi_e_timedata,1,'last');
            substructureEnds_index_e = substructureEndse_index;%find(fpi_coreTimedata(substructureEnds_index) <= fpi_e_timedata,1,'first');
            ...
            %Find the time of max density
            maxN_FPItime = fpi_e_coreTimedata(fpi_e_coreNdata == max(fpi_e_coreNdata(substructureStartse_index:substructureEndse_index)));
            SSStart_FPItime = fpi_e_coreTimedata(substructureStartse_index,1);
            SSEnd_FPItime = fpi_e_coreTimedata(substructureEndse_index,1);
            
            SSStart_FGMtime = find(mms1_fgm_timedata >= SSStart_FPItime,1);
            SSEnd_FGMtime = find(mms1_fgm_timedata >= SSEnd_FPItime,1);
            
            maxN_FGMtime=floor((SSStart_FGMtime+SSEnd_FGMtime)/2);%find(mms1_fgm_timedata >= maxN_FPItime,1);
            ...
            SS_n_e_values = fpi_e_coreNdata(substructureStartse_index:substructureEndse_index);
            SS_temp_e_values = (1/3).*(fpi_e_coretparadata(substructureStartse_index:substructureEndse_index) + ...
                2.*fpi_e_coretperpdata(substructureStartse_index:substructureEndse_index))
            %SS_n_e_values = fpi_e_ndata(find(fpi_e_timedata>...
            %    fpi_coreTimedata(substructureStarts_index),1):find(fpi_e_timedata>fpi_coreTimedata(substructureEnds_index)));
            ...
            %%Electron SkinDepth Calculation
            SS_n_e = 10^6 * mean(SS_n_e_values);
            ...
            %SS Size using Ion Inertial Length
            omega_pi=((10^6*mean(SS_n_e_values)*q^2)/(mion*epsilon_0))^(1/2);
            ion_skinDepth = (speedofLight/omega_pi) / 1000; %in km
            SSSize_in_IonScales = SSSizeBulkFlow/ion_skinDepth;
            
            
            ionGyroradius = ion_gyroradius;
            ionInertiallength = ion_skinDepth;
            
            %SS Size in Electron Gyroradii
            SSB = mean(mms1_fgm_bdata(SSStart_FGMtime:SSEnd_FGMtime,4))*10^-9;
            SSTperpe = mean(fpi_e_coretperpdata(substructureStartse_index:substructureEndse_index)); %Electron Perp Temp
            vperpe = (2*q*SSTperpe/(me))^(1/2);
            electron_gyroradius = (me)*vperpe / (q*SSB)/1000; %in km
            SSSizeBulkVflow_electronGyro= SSSizeBulkFlow/electron_gyroradius
            
            SSSizeBulkVflow_electronGyro = SSSizeBulkVflow_electronGyro;
            electronGyroradius = electron_gyroradius;
            electronInertiallength = electron_skinDepth;
            %             plot_fpi_number(left_InnerEdge,right_InnerEdge,fpi_coreTimedata,fpi_coreNdata,2,1,'i')
            %             plot_fpi_number(left_InnerEdge,right_InnerEdge,fpi_e_coreTimedata,fpi_e_coreNdata,2,2,'e')
            ...
            plot(fpi_e_coreTimedata,trendDensity,'g','linewidth',2);
            plot(fpi_e_coreTimedata,ssSTDs*Sigma+trendDensity,'g','linewidth',2);
            
            datetick
            set(gca,'XMinorTick','off','TickDir','out','YMinorTick','on','linewidth',2)
            fileName = 'coreDensities_yesSS_';
            title('Core Densities_yesSS_')
            % cd '~/Library/Mobile Documents/com~apple~CloudDocs/Research/Analyzed Events/All Events_Typical_Final'
            print(gcf,'-dpng','-r300', '-loose', strcat(fileName));
            savefig(strcat(fileName));
            %%
