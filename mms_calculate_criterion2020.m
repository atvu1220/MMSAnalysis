...
for i=TypicalEvents7'
    ...
    %[downstreamETperp,upstreamETPerp] = calculate_prepostAverages(fpi_e_timedata,fpi_e_tperpdata,event_start,event_end,'e',durationForAverage);
    [preMeanETperp,postMeanETperp] = calculate_ambientMean(fpi_e_timedata,fpi_e_tperpdata,event_start,left_OuterEdge,left_InnerEdge,right_InnerEdge,right_OuterEdge,event_end);  
    meanAmbientETperp = mean([preMeanETperp,postMeanETperp]);
    
    [meanCoreETpara] = calculate_meanDepletion(fpi_e_timedata,fpi_e_tparadata,left_InnerEdge,right_InnerEdge,0);
    %[downstreamETpara,upstreamETpara] = calculate_prepostAverages(fpi_e_timedata,fpi_e_tparadata,event_start,event_end,'e',durationForAverage);
    [preMeanETpara,postMeanETpara] = calculate_ambientMean(fpi_e_timedata,fpi_e_tparadata,event_start,left_OuterEdge,left_InnerEdge,right_InnerEdge,right_OuterEdge,event_end);
    meanAmbientETpara = mean([preMeanETpara,postMeanETpara]);
    
    %% Ratio of MAgnetic Field Strength
    bx = smooth(mms1_fgm_bdata_raw(:,1),128*10);
    by = smooth(mms1_fgm_bdata_raw(:,2),128*10);
    bz = smooth(mms1_fgm_bdata_raw(:,3),128*10);
    bmag = (bx.^2 + by.^2 + bz.^2).^(1/2);
    
    [meanCoreB]=calculate_meanCore(mms1_fgm_timedata_raw,bmag,left_InnerEdge,right_InnerEdge);
    [preMeanB,postMeanB] = calculate_ambientMean(mms1_fgm_timedata_raw,bmag,event_start,left_OuterEdge,left_InnerEdge,right_InnerEdge,right_OuterEdge,event_end);  
    BRatio = meanCoreB/mean([preMeanB,postMeanB]);
    
    %% Ratio of Electron Temperatures
    coreOverAmbientETperp = meanCoreETperp./meanAmbientETperp;
    coreOverAmbientETpara = meanCoreETpara./meanAmbientETpara;
    meanAmbientETtot = 1/3 .* (2*meanAmbientETperp + meanAmbientETpara);
    meanCoreETtot = 1/3 .* (2*meanCoreETperp + 2*meanCoreETpara);
    coreOverAmbientETtot = meanCoreETtot./meanAmbientETtot;
    %% Correlations
    ...
    criterionStructure.eTperpRatio(i) =  coreOverAmbientETperp;
    criterionStructure.eTparaRatio(i) =  coreOverAmbientETpara;
    criterionStructure.eTtotRatio(i) =  coreOverAmbientETtot;
    criterionStructure.BCoreOverAmbient(i) = BRatio;
...
plot_histograms({'Core Over Ambient eTperp Ratio'},0:0.5:5,criterionStructure.eTperpRatio, 'All');
plot_histograms({'Core Over Ambient eTpara Ratio'},0:0.5:5,criterionStructure.eTparaRatio, 'All');
plot_histograms({'Core Over Ambient eTtot Min Ratio'},0:0.5:5,criterionStructure.eTtotRatio, 'All');
plot_histograms({'Core Over Ambient B Strength Ratio'},0:0.25:3,criterionStructure.BCoreOverAmbient, '10s Smoothed');
...
function [meanCore] = calculate_meanCore(fpi_timedata,fpi_ndata,left_InnerEdge,right_InnerEdge)
    %Calculate max or min.
    formatIn='yyyy-mm-dd HH:MM:SS.FFF';
...
function [preMean,postMean] = calculate_ambientMean(timedata,data,event_start,left_OuterEdge,left_InnerEdge,right_InnerEdge,right_OuterEdge,event_end)
    ...
    % Time limits
    coreStart = datenum(left_InnerEdge,formatIn);
    coreStart_index = find(timedata > coreStart, 1);
    coreEnd = datenum(right_InnerEdge,formatIn);
    coreEnd_index = find(timedata > coreEnd, 1);
