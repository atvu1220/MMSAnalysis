...
date_start = '2018-03-01 01:05:52.250';
date_end = '2018-03-01 01:05:53.750';
...
MVAB_window_size = 1024; %half second on each side. %16 for srvy, 128 for brst;;;0 for default window size of the time interval
...
% %group compoinents of B and r together for faster loops.
% bx=[smoothen(mms1_fgm_bdata_interp(:,1),3) smoothen(mms2_fgm_bdata_interp(:,1),3) smoothen(mms3_fgm_bdata_interp(:,1),3) smoothen(mms4_fgm_bdata_interp(:,1),1)];
% by=[smoothen(mms1_fgm_bdata_interp(:,2),3) smoothen(mms2_fgm_bdata_interp(:,2),3) smoothen(mms3_fgm_bdata_interp(:,2),3) smoothen(mms4_fgm_bdata_interp(:,2),1)];
% bz=[smoothen(mms1_fgm_bdata_interp(:,3),3) smoothen(mms2_fgm_bdata_interp(:,3),3) smoothen(mms3_fgm_bdata_interp(:,3),3) smoothen(mms4_fgm_bdata_interp(:,3),1)];
%
% rx=[smoothen(mms1_fgm_rdata_interp(:,1),3) smoothen(mms2_fgm_rdata_interp(:,1),3) smoothen(mms3_fgm_rdata_interp(:,1),3) smoothen(mms4_fgm_rdata_interp(:,1),1)];
% ry=[smoothen(mms1_fgm_rdata_interp(:,2),3) smoothen(mms2_fgm_rdata_interp(:,2),3) smoothen(mms3_fgm_rdata_interp(:,2),3) smoothen(mms4_fgm_rdata_interp(:,2),1)];
% rz=[smoothen(mms1_fgm_rdata_interp(:,3),3) smoothen(mms2_fgm_rdata_interp(:,3),3) smoothen(mms3_fgm_rdata_interp(:,3),3) smoothen(mms4_fgm_rdata_interp(:,3),1)];
...
%Find the largest value for the smallest eigenvalue, corresponds to largest G.
...
MVA_dot_MDD_center = min(MVA_ambig)
...
for i=1:length(rx)
    %fix 180 degrees ambiguity for one direction
    MVA_ambig_dot(1)=rad2deg(acos(dot(MVAB_normal_all(i,:).*[-1,1,1],MDD_mean_normal)/(norm(MVAB_normal_all(i,:))*norm(MDD_mean_normal))));
    MVA_ambig_dot(2)=rad2deg(acos(dot(MVAB_normal_all(i,:).*[1,-1,1],MDD_mean_normal)/(norm(MVAB_normal_all(i,:))*norm(MDD_mean_normal))));
    MVA_ambig_dot(3)=rad2deg(acos(dot(MVAB_normal_all(i,:).*[1,1,-1],MDD_mean_normal)/(norm(MVAB_normal_all(i,:))*norm(MDD_mean_normal))));
    MVA_ambig_dot(4)=rad2deg(acos(dot(MVAB_normal_all(i,:),MDD_mean_normal)/(norm(MVAB_normal_all(i,:))*norm(MDD_mean_normal))));
...
set(gca,'layer','top')
legend({'Sliding MVA', 'Centered MVA'},'FontSize',10)
legend('boxoff');
legend('Location','eastoutside');
%xlim([datetime(tstart,'ConvertFrom','datenum') datetime(tend,'ConvertFrom','datenum')]);
...
%Testing grad_b of all terms
AA = sum(squeeze(grad_B(1,1,:)));
BB = sum(squeeze(grad_B(1,2,:)));
CC = sum(squeeze(grad_B(1,3,:)));
DD = sum(squeeze(grad_B(2,1,:)));
EE = sum(squeeze(grad_B(2,2,:)));
FF = sum(squeeze(grad_B(2,3,:)));
GG = sum(squeeze(grad_B(3,1,:)));
HH = sum(squeeze(grad_B(3,2,:)));
II = sum(squeeze(grad_B(3,3,:)));
...
annotation('textbox',[plot_pos(1), plot_pos(2)-8*plot_pos(4)-.15, plot_pos(3), plot_pos(4)*1],...
...
annotation('textbox',[plot_pos(1), plot_pos(2)-8*plot_pos(4)-0.20, plot_pos(3), plot_pos(4)*1],...
    'String',strcat('(Mean MDD)\cdot(Centered MVA) Angle= ', ' ', num2str(MVA_dot_MDD_center,'%2.2f'),char(176)),...
    'VerticalAlignment','Bottom','Edgecolor','none','FontSize', 14);

annotation('textbox',[plot_pos(1)+0.55*plot_pos(3), plot_pos(2)-8*plot_pos(4)-0.180, plot_pos(3), plot_pos(4)*1],...
    'String',strcat('Median MDD Normal = ', ' ', num2str(median(MDDB_normal),' [%0.4f, %0.4f, %0.4f]')),...
    'VerticalAlignment','Bottom','Edgecolor','none','FontSize', 14);
...
annotation('textbox',[plot_pos(1)+0.55*plot_pos(3), plot_pos(2)-8*plot_pos(4)-0.22, plot_pos(3), plot_pos(4)*1],...
...
plot_name =  strcat('mms','_mdd_',mddl,'_mva_',mval,'_angles',...
    date_start(1:19),'_avg_i',num2str(avg_i),'_fluctmdd',num2str(fluctuate_mdd),'_fluctmva',num2str(fluctuate_mva),'_MVAB_window',num2str(MVAB_window_size),'.pdf');
