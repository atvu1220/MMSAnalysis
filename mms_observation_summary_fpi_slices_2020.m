...
for Rotation = [{'xy'},{'xz'},{'BE'},{'BV'},{'perp'}]
    ...
    for probe = [1,2,3,4]
        ...
        set(gcf,'color','w');
        plot_gap = 1.00;
        cd '~/Library/Mobile Documents/com~apple~CloudDocs/Research/Case Studies/'
        ...
        %event_end = '2019-02-22 12:01:39.000';
        ...
        event_end = '2019-02-23 06:06:10.000';
        
        % probe = 3;
        %slices_directory = strcat('~/Library/Mobile Documents/com~apple~CloudDocs/Research/Analysis/Velocity Distributions/' ,'EE',num2str(EventNumber),'/',num2str(probe),Rotation);
        slices_directory = strcat('~/Library/Mobile Documents/com~apple~CloudDocs/Research/Case Studies/Velocity Distributions/',num2str(EventNumber),'/',num2str(probe),Rotation);
        
        
        % EventNumber = 0;
        % if EventNumber == 0
        %     trange = {'2019-02-16 05:40:40.000','2019-02-16 05:42:10.000'};
        ...
        title(strcat('MMS',probe_num,{' '},'Observatory FPI Summary'), 'FontSize', 18, 'FontWeight', 'normal')
        ...
        whitejet = [1 1 1; jet];
        colormap(whitejet);
        ylabel({'Energy'; 'Spectrum';'[eV]'},'FontSize', 13)
        ...
        all_filenames = all_filenames(~contains(all_filenames,'.mp4'));
        ...
        vidfile = VideoWriter(strcat(num2str(EventNumber),'_VelocityDistribution_',num2str(probe),Rotation,'.mp4'),'MPEG-4');
        vidfile.FrameRate = 4;
        open(vidfile);
        
        for i=1:numberOfSlices
            filename = char(all_filenames(i));
            timeMark = datenum(filename(1:15),formatIn);
            axes(Bplot)
            Bline = line([timeMark,timeMark],get(gca,'YLim'),'Color','r','LineWidth',2,'LineStyle','--');
            axes(nplot)
            nlimits = ylim;
            set(gca,'ylim',nlimits);
            nline = line([timeMark,timeMark],get(gca,'YLim'),'Color','r','LineWidth',2,'LineStyle','--');
            
            axes(vplot)
            vlimits = ylim;
            set(gca,'ylim',vlimits);
            vline = line([timeMark,timeMark],vlimits,'Color','r','LineWidth',2,'LineStyle','--');
